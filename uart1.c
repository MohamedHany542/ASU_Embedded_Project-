...
static void GPIO_SetupUART1Pins(void)
{
    ...
    GPIO_PORTB_AMSEL_REG &= 0xFC;         /* Disable Analog on PB0 & PB1 */
...
void UART1_Init(void)
{
    ...
    SYSCTL_RCGCUART_REG |= 0x01;          /* Enable clock for UART1 */
    while(!(SYSCTL_PRUART_REG & 0x01));   /* Wait until UART1 clock is activated and it is ready for access*/
     ...
     */
    UART1_LCRH_REG = (UART_DATA_8BITS << UART_LCRH_WLEN_BITS_POS);
